...
void ssd_write(ScreenDefines Screen, size_t data_length){
    ...
    if(!i2c.IsBusy()){
        ...
        level_log(TRACE, "SSD1306: I2C Write Pending...First byte is supposed to: %X", Screen.pbuffer[0]);
        
        if(i2c.Write(Screen.i2c_address, Screen.pbuffer, data_length)){
            level_log(WARNING, "SSD1306: I2C Bus Busy");
        }

        // level_log(TRACE, "SSD1306: I2C Write Sent to Peripheral");
        ...
        if(!i2c.IsBusy())
        {
            ...
            {
                // I2C Write sucessful
                level_log(INFO, "SSD1306: I2C Write Sucessful");
            }
            else
            {
                // Error handling 
                level_log(ERROR, "SSD1306: I2C Write Error");
            }
        }
    } 
    else 
    {
        level_log(ERROR, "SSD1306: I2C Cannot Write to Peripheral. Bus is Busy");
    }

    level_log(TRACE, "SSD1306: I2C Write Finished");
...
void ssd_write(size_t data_length){
    ...
    if(!i2c.IsBusy()){
        ...
        level_log(TRACE, "SSD1306: I2C Write Pending...First byte is supposed to: %X", ssd1306_i2c_buffer[0]);
        
        if(i2c.Write(SSD1306_ADDRESS, &ssd1306_i2c_buffer, data_length)){
            level_log(WARNING, "SSD1306: I2C Bus Busy");
        }
...
size_t load_i2c_buffer(ScreenDefines Screen, uint8_t * psetup_bytes, size_t setup_length, uint8_t * pdata, size_t data_length) {
    ...
    level_log(TRACE, "SSD1306: Loading the I2C Buffer");
    ...
    level_log(TRACE, "SSD1306: I2C Buffer Loaded");
